'use client';

import { useState, useEffect } from 'react';
import { AuthProvider, useAuth } from '@/store/AuthContext';
import Sidebar from '@/components/layout/Sidebar';
import { api } from '@/lib/api';
import { Tweet } from '@/types';
import { useRouter } from 'next/navigation';

function TweetDetail({ params }: { params: { id: string } }) {
  const { user } = useAuth();
  const router = useRouter();
  const [tweet, setTweet] = useState<Tweet | null>(null);
  const [replies, setReplies] = useState<any[]>([]);
  const [replyContent, setReplyContent] = useState('');

  useEffect(() => {
    api.get(`/tweet/${params.id}`).then(({ data }) => {
      if (data.success) setTweet(data.data);
    });
    api.get(`/tweet/replies/${params.id}`).then(({ data }) => {
      if (data.success) setReplies(data.data);
    });
  }, [params.id]);

  const handleLike = async () => {
    if (!tweet) return;
    try {
      await api.post(`/tweet/like/${tweet.id}`);
      setTweet((prev) => prev ? { ...prev, likeCount: prev.likeCount + 1 } : null);
    } catch {}
  };

  const handleReply = async () => {
    if (!replyContent.trim()) return;
    try {
      const { data } = await api.post('/tweet/reply', { tweetId: params.id, content: replyContent });
      if (data.success) {
        setReplies((prev) => [...prev, data.data]);
        setReplyContent('');
      }
    } catch {}
  };

  if (!tweet) return (
    <div className="flex min-h-screen bg-black">
      <Sidebar />
      <main className="flex-1 flex items-center justify-center">
        <div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin" />
      </main>
    </div>
  );

  return (
    <div className="flex min-h-screen bg-black">
      <Sidebar />
      <main className="flex-1 border-r border-gray-800 max-w-2xl">
        <div className="sticky top-0 z-10 backdrop-blur-md bg-black/70 border-b border-gray-800">
          <div className="flex items-center gap-4 px-6 py-4">
            <button onClick={() => router.back()} className="w-9 h-9 rounded-full flex items-center justify-center hover:bg-gray-800 transition">
              <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            </button>
            <h1 className="text-lg font-bold text-white">Post</h1>
          </div>
        </div>

        <div className="p-6 border-b border-gray-800">
          <div className="flex items-start gap-4 mb-4">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-primary to-purple-600 flex items-center justify-center text-white font-bold text-lg shrink-0 shadow-lg shadow-primary/30">
              {tweet.authorId?.[0]?.toUpperCase() || '?'}
            </div>
            <div>
              <p className="font-bold text-white">@{tweet.authorId?.slice(0, 8)}</p>
              <p className="text-sm text-gray-500">{new Date(tweet.createdAt).toLocaleString()}</p>
            </div>
          </div>
          <p className="text-xl leading-relaxed text-gray-100 mb-4 whitespace-pre-wrap">{tweet.content}</p>
          <div className="flex items-center gap-6 text-gray-500 border-t border-gray-800 pt-4">
            <button onClick={handleLike} className="flex items-center gap-2 text-sm hover:text-pink-500 transition group">
              <svg className="w-5 h-5 group-hover:scale-110 transition" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
              <span>{tweet.likeCount}</span>
            </button>
            <span className="flex items-center gap-2 text-sm">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
              <span>{tweet.replyCount}</span>
            </span>
            <span className="flex items-center gap-2 text-sm">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
              <span>{tweet.retweetCount}</span>
            </span>
            <span className="flex items-center gap-2 text-sm">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" /></svg>
              <span>{tweet.bookmarkCount}</span>
            </span>
          </div>
        </div>

        <div className="p-4 border-b border-gray-800">
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-gray-600 to-gray-700 flex items-center justify-center text-white font-bold shrink-0 shadow-lg">
              {user?.username?.[0]?.toUpperCase() || '?'}
            </div>
            <div className="flex-1">
              <textarea placeholder="Post your reply" value={replyContent} onChange={(e) => setReplyContent(e.target.value)}
                className="w-full bg-transparent text-white placeholder-gray-600 resize-none outline-none text-base" rows={3} maxLength={280} />
              <div className="flex justify-end border-t border-gray-800 pt-3">
                <button onClick={handleReply} disabled={!replyContent.trim()}
                  className="btn-primary py-2 px-5 text-sm">Reply</button>
              </div>
            </div>
          </div>
        </div>

        <div className="divide-y divide-gray-800/50">
          {replies.map((r: any) => (
            <div key={r.id} className="p-5 hover:bg-gray-900/30 transition">
              <div className="flex gap-4">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-gray-600 to-gray-700 shrink-0 shadow-lg" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 text-sm">
                    <span className="font-bold text-white">@{r.authorId?.slice(0, 8)}</span>
                    <span className="text-gray-600">·</span>
                    <span className="text-gray-500">{new Date(r.createdAt).toLocaleDateString()}</span>
                  </div>
                  <p className="text-[15px] leading-relaxed text-gray-200 mt-1 whitespace-pre-wrap">{r.content}</p>
                </div>
              </div>
            </div>
          ))}
          {!replies.length && (
            <div className="p-12 text-center">
              <p className="text-gray-500">No replies yet</p>
            </div>
          )}
        </div>
      </main>
    </div>
  );
}

export default function TweetPage({ params }: { params: { id: string } }) {
  return <AuthProvider><TweetDetail params={params} /></AuthProvider>;
}
